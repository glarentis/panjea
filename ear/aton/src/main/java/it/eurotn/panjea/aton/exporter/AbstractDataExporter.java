...
public abstract class AbstractDataExporter {
	...
	protected File getFileForExport() throws FileCreationException {
		...
		try {
			...
			query.setParameter("paramChiave", "atonDirExport");
			preference = (Preference) panjeaDAO.getSingleResult(query);
		} catch (Exception e) {
			logger.error("--> Errore ricerca preference con key atonDirExport", e);
			throw new FileCreationException("Nelle preferenze generali manca la chiave atonDirExport");
		}

		// Costruisco il nome del file
		String fileName = preference.getValore() + getClass().getSimpleName().replace("Exporter", "") + ".TOT";
	...
	protected File getFilePathForTemplate() throws FileCreationException {
		...
		// Costruisco il file
		String fileName = preference.getValore() + getClass().getSimpleName().replace("Exporter", "") + ".xml";
