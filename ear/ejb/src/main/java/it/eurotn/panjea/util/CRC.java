 ...
 * <p>
 *
 * This class inherits java.util.zip.CRC32 - so the real power is there. This class replaces update(int) because the
 * CRC32.update(int) considers only the first byte. This class considers all four bytes of the int.
 * <p>
 *
 * Further, this class adds update(String), update(byte), update(char), update(long), update(double), update(float) and
 * update(boolean) for convenience.
 * <p>
 *
 ...
 * ... sell it, share it ... add a copyright for the portions you add ...
 * <p>
 *
...
public class CRC extends CRC32 {
	 ...
	 *            s
	 *
	 * @return an unsigned 31 bit CRC value (a 32 bit CRC with the sign bit chopped off).
	 *         <p>
	 */
	 ...
	 * <p>
	 *
	 * This is useful because it avoids the problems some code might have with a negative value. Plus, it makes for a
	 * clean and consistant conversion to an int as used for hashCode().
	 * <p>
	 *
	 * @return an unsigned 31 bit CRC value (a 32 bit CRC with the sign bit chopped off).
	 *         <p>
	 ...
	 *
	 * @param b
	 *            a boolean to work into the CRC code. true is worked in as a byte of value 1 and false is worked in as
	 *            a byte of value 0.
	 *            <p>
	 */
	 ...
	 *
	 * @param b
	 *            a byte to work into the CRC code. All eight bits are worked in.
	 *            <p>
	 */
	 ...
	 *
	 * @param c
	 *            a char to work into the CRC code. Both bytes are worked in.
	 *            <p>
	 */
	 ...
	 *
	 * @param d
	 *            a double precision float to work into the CRC code. All eight bytes are worked in.
	 *            <p>
	 */
	 ...
	 *
	 * @param f
	 *            a floating point value to work into the CRC code. All four bytes are worked in.
	 *            <p>
	 */
	...
	/**
	 * work a {@link IDefProperty} into the existing CRC code.
	 *
	 * @param defProperty
	 *            a {@link IDefProperty} to work into the CRC code.
	 */
	public void update(IDefProperty defProperty) {
		if (defProperty != null && defProperty.getId() != null) {
			update(defProperty.getId().intValue());
		}
	}
	 ...
	 *
	 * @param i
	 *            an integer to work into the CRC code. All four bytes are worked in.
	 *            <p>
	 */
	@Override
	 ...
	 *
	 * @param i
	 *            a long integer to work into the CRC code. All eight bytes are worked in.
	 *            <p>
	 */
	 ...
	 *
	 * @param s
	 *            a string to work into the CRC code. Two bytes for every character is worked in. The length is not
	 *            directly worked in.
	 *            <p>
	 */
