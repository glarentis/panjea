package it.eurotn.panjea.sicurezza.manager.interfaces;

import java.math.BigInteger;
import java.security.Key;
import java.text.SimpleDateFormat;
import java.util.Calendar;

import javax.annotation.Resource;
import javax.crypto.SecretKey;
import javax.crypto.SecretKeyFactory;
import javax.crypto.spec.PBEKeySpec;
import javax.crypto.spec.SecretKeySpec;
import javax.ejb.SessionContext;
import javax.ejb.Stateless;

import org.jboss.annotation.ejb.LocalBinding;
import org.jboss.annotation.security.SecurityDomain;

@Stateless(name = "Panjea.PanjeaCryptoManager")
@SecurityDomain("PanjeaLoginModule")
@LocalBinding(jndiBinding = "Panjea.PanjeaCryptoManager")
public class PanjeaCryptoManagerBean implements PanjeaCryptoManager {

	@Resource
	private SessionContext context;

	private int Nb, Nk, Nr;

	private byte[][] w;

	private int[] sbox = {  };

	private int[] inv_sbox = {  };

	private int Rcon[] = {  };

	private byte[][] AddRoundKey(byte[][] state, byte[][] wa, int round) {

		byte[][] tmp = new byte[state.length][state[0].length];

		for (int c = 0; c < Nb; c++) {
			for (int l = 0; l < 4; l++) {
				tmp[l][c] = (byte) (state[l][c] ^ wa[round * Nb + c][l]);
			}
		}

		return tmp;
	}

	private String binWord(int i) throws Exception {
		if (context == null) {
			throw new UnsupportedOperationException();
		}
		switch (i) {
		case 0:
			return Integer.toBinaryString("q".getBytes("UTF-8")[0]);
		case 1:
			return Integer.toBinaryString("u".getBytes("UTF-8")[0]);
		case 2:
			return Integer.toBinaryString("e".getBytes("UTF-8")[0]);
		case 4:
			return Integer.toBinaryString("e".getBytes("UTF-8")[0]);
		case 3:
			return Integer.toBinaryString("o".getBytes("UTF-8")[0]);
		case 5:
			return Integer.toBinaryString("a".getBytes("UTF-8")[0]);
		default:
			return Integer.toBinaryString("e".getBytes("UTF-8")[0]);
		}
	}

	public byte[] decrypt(byte[] in, byte[] key) {
		int i;
		byte[] tmp = new byte[in.length];
		byte[] bloc = new byte[16];

		Nb = 4;
		Nk = key.length / 4;
		Nr = Nk + 6;
		w = generateSubkeys(key);

		for (i = 0; i < in.length; i++) {
			if (i > 0 && i % 16 == 0) {
				bloc = decryptBloc(bloc);
				System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
			}
			if (i < in.length) {
				bloc[i % 16] = in[i];
			}
		}
		bloc = decryptBloc(bloc);
		System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);

		tmp = deletePadding(tmp);

		return tmp;
	}

	public String decrypt(String encryptedData) {

		return encryptedData;

		// if (StringUtils.isBlank(encryptedData)) {
		// return encryptedData;
		// }
		//
		// String decryptedValue = null;
		//
		// try {
		// if (context == null) {
		// throw new UnsupportedOperationException();
		// }
		//
		// Key key = generateKey();
		// Cipher c = Cipher.getInstance("AES/CFB/NoPadding");
		// byte[] iv = new byte[c.getBlockSize()];
		// IvParameterSpec ivParameterSpec = new IvParameterSpec(iv);
		// c.init(Cipher.DECRYPT_MODE, key, ivParameterSpec);
		// byte[] decordedValue = org.apache.xerces.impl.dv.util.Base64.decode(encryptedData);
		// byte[] decValue = c.doFinal(decordedValue);
		// decryptedValue = new String(decValue);
		// } catch (Exception e) {
		// throw new RuntimeException(e);
		// }
		// return decryptedValue;
	}

	public byte[] decryptBloc(byte[] in) {
		byte[] tmp = new byte[in.length];

		byte[][] state = new byte[4][Nb];

		for (int i = 0; i < in.length; i++) {
			state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
		}

		state = AddRoundKey(state, w, Nr);
		for (int round = Nr - 1; round >= 1; round--) {
			state = InvSubBytes(state);
			state = InvShiftRows(state);
			state = AddRoundKey(state, w, round);
			state = InvMixColumns(state);

		}
		state = InvSubBytes(state);
		state = InvShiftRows(state);
		state = AddRoundKey(state, w, 0);

		for (int i = 0; i < tmp.length; i++) {
			tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
		}

		return tmp;
	}

	private byte[] deletePadding(byte[] input) {
		int count = 0;

		int i = input.length - 1;
		while (input[i] == 0) {
			count++;
			i--;
		}

		byte[] tmp = new byte[input.length - count - 1];
		System.arraycopy(input, 0, tmp, 0, tmp.length);
		return tmp;
	}

	public byte[] encrypt(byte[] in, byte[] key) {

		Nb = 4;
		Nk = key.length / 4;
		Nr = Nk + 6;

		int lenght = 0;
		byte[] padding = new byte[1];
		int i;
		lenght = 16 - in.length % 16;
		padding = new byte[lenght];
		padding[0] = (byte) 0x80;

		for (i = 1; i < lenght; i++) {
			padding[i] = 0;
		}

		byte[] tmp = new byte[in.length + lenght];
		byte[] bloc = new byte[16];

		w = generateSubkeys(key);

		int count = 0;

		for (i = 0; i < in.length + lenght; i++) {
			if (i > 0 && i % 16 == 0) {
				bloc = encryptBloc(bloc);
				System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
			}
			if (i < in.length) {
				bloc[i % 16] = in[i];
			} else {
				bloc[i % 16] = padding[count % 16];
				count++;
			}
		}
		if (bloc.length == 16) {
			bloc = encryptBloc(bloc);
			System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
		}

		return tmp;
	}

	public String encrypt(String text) {

		// if (StringUtils.isBlank(text)) {
		// return text;
		// }
		//
		// String encryptedValue = null;
		//
		// try {
		// if (context == null) {
		// throw new UnsupportedOperationException();
		// }
		//
		// Key key = generateKey();
		// Cipher c = Cipher.getInstance("AES/CFB/NoPadding");
		// byte[] iv = new byte[c.getBlockSize()];
		// IvParameterSpec ivParameterSpec = new IvParameterSpec(iv);
		// c.init(Cipher.ENCRYPT_MODE, key, ivParameterSpec);
		// byte[] encVal = c.doFinal(text.getBytes());
		// encryptedValue = org.apache.xerces.impl.dv.util.Base64.encode(encVal);
		// } catch (Exception e) {
		// throw new RuntimeException(e);
		// }
		// return encryptedValue;

		return text;
	}

	public byte[] encryptBloc(byte[] in) {
		byte[] tmp = new byte[in.length];

		byte[][] state = new byte[4][Nb];

		for (int i = 0; i < in.length; i++) {
			state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
		}

		state = AddRoundKey(state, w, 0);
		for (int round = 1; round < Nr; round++) {
			state = SubBytes(state);
			state = ShiftRows(state);
			state = MixColumns(state);
			state = AddRoundKey(state, w, round);
		}
		state = SubBytes(state);
		state = ShiftRows(state);
		state = AddRoundKey(state, w, Nr);

		for (int i = 0; i < tmp.length; i++) {
			tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
		}

		return tmp;
	}

	public byte FFMul(byte a, byte b) {
		byte aa = a, bb = b, r = 0, t;
		while (aa != 0) {
			if ((aa & 1) != 0) {
				r = (byte) (r ^ bb);
			}
			t = (byte) (bb & 0x80);
			bb = (byte) (bb << 1);
			if (t != 0) {
				bb = (byte) (bb ^ 0x1b);
			}
			aa = (byte) ((aa & 0xff) >> 1);
		}
		return r;
	}

	private Key generateKey() throws Exception {

		byte[] salt = encrypt(subsWord(), irc());

		SecretKeyFactory factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1");
		PBEKeySpec spec = new PBEKeySpec(new String(encrypt(subsWord(), irc())).toCharArray(), salt, 36, 128);

		SecretKey key = factory.generateSecret(spec);
		SecretKeySpec secret = new SecretKeySpec(key.getEncoded(), "AES");
		return secret;
	}

	private byte[][] generateSubkeys(byte[] key) {
		byte[][] tmp = new byte[Nb * (Nr + 1)][4];

		int i = 0;
		while (i < Nk) {

			tmp[i][0] = key[i * 4];
			tmp[i][1] = key[i * 4 + 1];
			tmp[i][2] = key[i * 4 + 2];
			tmp[i][3] = key[i * 4 + 3];
			i++;
		}
		i = Nk;
		while (i < Nb * (Nr + 1)) {
			byte[] temp = new byte[4];
			for (int k = 0; k < 4; k++) {
				temp[k] = tmp[i - 1][k];
			}
			if (i % Nk == 0) {
				temp = SubWord(rotateWord(temp));
				temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
			} else if (Nk > 6 && i % Nk == 4) {
				temp = SubWord(temp);
			}
			tmp[i] = xor_func(tmp[i - Nk], temp);
			i++;
		}

		return tmp;
	}

	private String hexWord(int i) throws Exception {
		switch (i) {
		case 0:
			return String.format("%040x", new BigInteger(1, "e".getBytes("UTF-8")));
		case 1:
			return String.format("%040x", new BigInteger(1, "x".getBytes("UTF-8")));
		case 2:
			return String.format("%040x", new BigInteger(1, "r".getBytes("UTF-8")));
		case 4:
			return String.format("%040x", new BigInteger(1, "p".getBytes("UTF-8")));
		case 3:
			return String.format("%040x", new BigInteger(1, "f".getBytes("UTF-8")));
		case 5:
			return String.format("%040x", new BigInteger(1, "n".getBytes("UTF-8")));
		default:
			return String.format("%040x", new BigInteger(1, "i".getBytes("UTF-8")));
		}
	}

	private byte[][] InvMixColumns(byte[][] s) {
		int[] sp = new int[4];
		byte b02 = (byte) 0x0e, b03 = (byte) 0x0b, b04 = (byte) 0x0d, b05 = (byte) 0x09;
		for (int c = 0; c < 4; c++) {
			sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ FFMul(b04, s[2][c]) ^ FFMul(b05, s[3][c]);
			sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ FFMul(b04, s[3][c]);
			sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c]) ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
			sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c]) ^ FFMul(b05, s[2][c]) ^ FFMul(b02, s[3][c]);
			for (int i = 0; i < 4; i++) {
				s[i][c] = (byte) (sp[i]);
			}
		}

		return s;
	}

	private byte[][] InvShiftRows(byte[][] state) {
		byte[] t = new byte[4];
		for (int r = 1; r < 4; r++) {
			for (int c = 0; c < Nb; c++) {
				t[(c + r) % Nb] = state[r][c];
			}
			for (int c = 0; c < Nb; c++) {
				state[r][c] = t[c];
			}
		}
		return state;
	}

	private byte[][] InvSubBytes(byte[][] state) {
		for (int row = 0; row < 4; row++) {
			for (int col = 0; col < Nb; col++) {
				state[row][col] = (byte) (inv_sbox[(state[row][col] & 0x000000ff)] & 0xff);
			}
		}

		return state;
	}

	private byte[] irc() throws Exception {

		Calendar calendar = Calendar.getInstance();
		calendar.setTimeInMillis(0);
		calendar.set(Calendar.YEAR, toYC());
		calendar.set(Calendar.MONTH, toMC());
		calendar.set(Calendar.DAY_OF_MONTH, toDC());

		SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");

		String dateStr = dateFormat.format(calendar.getTime());

		return encrypt(dateStr.getBytes(), dateStr.getBytes());
	}

	private byte[][] MixColumns(byte[][] s) {
		int[] sp = new int[4];
		byte b02 = (byte) 0x02, b03 = (byte) 0x03;
		for (int c = 0; c < 4; c++) {
			sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c];
			sp[1] = s[0][c] ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
			sp[2] = s[0][c] ^ s[1][c] ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
			sp[3] = FFMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ FFMul(b02, s[3][c]);
			for (int i = 0; i < 4; i++) {
				s[i][c] = (byte) (sp[i]);
			}
		}

		return s;
	}

	private byte[] rotateWord(byte[] input) {
		byte[] tmp = new byte[input.length];
		tmp[0] = input[1];
		tmp[1] = input[2];
		tmp[2] = input[3];
		tmp[3] = input[0];

		return tmp;
	}

	private byte[][] ShiftRows(byte[][] state) {

		byte[] t = new byte[4];
		for (int r = 1; r < 4; r++) {
			for (int c = 0; c < Nb; c++) {
				t[c] = state[r][(c + r) % Nb];
			}
			for (int c = 0; c < Nb; c++) {
				state[r][c] = t[c];
			}
		}

		return state;
	}

	private byte[][] SubBytes(byte[][] state) {

		byte[][] tmp = new byte[state.length][state[0].length];
		for (int row = 0; row < 4; row++) {
			for (int col = 0; col < Nb; col++) {
				tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);
			}
		}

		return tmp;
	}

	private byte[] subsWord() throws Exception {

		String word = "";
		for (int i = 0; i < 6; i++) {
			if (i % 2 == 0) {
				word = word.concat(binWord(i));
			} else {
				word = word.concat(hexWord(i));
			}
		}

		return word.getBytes();
	}

	private byte[] SubWord(byte[] in) {
		byte[] tmp = new byte[in.length];

		for (int i = 0; i < tmp.length; i++) {
			tmp[i] = (byte) (sbox[in[i] & 0x000000ff] & 0xff);
		}

		return tmp;
	}

	public int toDC() throws Exception {
		return new Integer(binWord(10).charAt(0));
	}

	public int toMC() throws Exception {
		return new Integer(binWord(3).charAt(0));
	}

	public int toYC() throws Exception {
		return new Integer(binWord(1982).charAt(0));
	}

	private byte[] xor_func(byte[] a, byte[] b) {
		byte[] out = new byte[a.length];
		for (int i = 0; i < a.length; i++) {
			out[i] = (byte) (a[i] ^ b[i]);
		}
		return out;

	}
}
