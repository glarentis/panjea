...
public class Sezione2AcquistiBuilder extends SezioneRigheBuilder<RigaSezione2Intra> {

	private static Logger logger = Logger.getLogger(Sezione2AcquistiBuilder.class);
	...
	private Map<Long, RigaSezioneIntra> aggiornaConRigheAddebito(Map<Long, RigaSezioneIntra> righe,
		...
		try {
			...
			throw new RuntimeException("errore nel recuperare le righeBene per la sezione 2", e);
		}
	...
	private Map<Long, RigaSezioneIntra> getRigheStorno(PanjeaDAO panjeaDAO, DichiarazioneIntra dichiarazioneIntra) {
		...
		sb.append("sum(rb.importo.importoInValuta) * -1 as importoInValuta, ");
		sb.append("intra.naturaTransazione as naturaTransazione, ");
		sb.append("servizi as nomenclatura, ");
