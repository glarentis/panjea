...
public final class EvasioneOrdiniQueryBuilder {
    ...
    private static String getFrom(ParametriRicercaEvasione parametri) {
    ...
    /**
     * Restituisce l'HQL per il caricamento delle righe da evadere o da associare ad una distinta di
     * carico.
     *
     * @param parametri
    ...
    public static String getHQL(ParametriRicercaEvasione parametri) {
        ...
        if (!parametri.getNumeroDocumentoIniziale().isEmpty()) {
            sb.append(" documento.codice.codiceOrder >= :numeroDocumentoIniziale and ");
        }

        if (!parametri.getNumeroDocumentoFinale().isEmpty()) {
            sb.append(" documento.codice.codiceOrder <= :numeroDocumentoFinale and ");
        }
        ...
        if (parametri.getDataInizioTrasporto() != null) {
            sb.append(" areaOrdine.dataInizioTrasporto = :dataInizioTrasporto and ");
        }
        sb.append(" areaOrdine.statoAreaOrdine = 1 and ");
        sb.append(" riga.bloccata = false and ");
        sb.append(
                " (righeDistintaCarico = null or (righeDistintaCarico != null and articolo.gestioneQuantitaZero = false)) ");
        sb.append(" group by riga ");
        sb.append(
                " having ((coalesce(sum(rigacoll.qta*rigacoll.moltQtaOrdine),0) + coalesce(sum(righeDistintaCarico.qtaDaEvadere),0)) < riga.qta) ");
        sb.append(
                " or (riga.qta=0 and (coalesce(sum(rigacoll.qta*rigacoll.moltQtaOrdine),0) + coalesce(sum(righeDistintaCarico.qtaDaEvadere),0) = 0))");
    ...
    /**
     * Avvalora tutti i parametri della query.
     *
     * @param query
     *            query
     * @param parametri
     *            parametri
     */
    public static void setParameters(HibernateQuery query, ParametriRicercaEvasione parametri) {
        query.getHibernateQuery().setProperties(parametri);

        if (!parametri.getNumeroDocumentoIniziale().isEmpty()) {
            query.setParameter("numeroDocumentoIniziale", parametri.getNumeroDocumentoIniziale().getCodiceOrder());
        }

        if (!parametri.getNumeroDocumentoFinale().isEmpty()) {
            query.setParameter("numeroDocumentoFinale", parametri.getNumeroDocumentoFinale().getCodiceOrder());
        }

        if (parametri.getDataInizioTrasporto() != null) {
            query.setParameter("dataInizioTrasporto", parametri.getDataInizioTrasporto());
        }
    }
}
