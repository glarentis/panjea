...
@Stateless(name = "Panjea.Cosaro.Evasione")
@SecurityDomain("PanjeaLoginModule")
@LocalBinding(jndiBinding = "Panjea.Cosaro.Evasione")
public class EvasioneCosaroServiceBean implements EvasioneCosaroService {
    ...
    private static Logger logger = Logger.getLogger(EvasioneCosaroServiceBean.class);
    ...
    private RigaDistintaCarico caricaRigaDistintaCarico(RigaEvasione rigaEvasione) {
        ...
        if (!righeDistintaCaricate.containsKey(rigaEvasione.getChiave())) {
            ...
            sb.append("inner join ra.articolo art ");
            ...
            sb.append("and ra.numeroRiga=:numeroRiga ");
            ...
            query.setParameter("numeroRiga", rigaEvasione.getNumRiga());
    ...
    private void evadiFile(File fileEvasione) {
        ...
        try {
            StreamFactory factory = cosaroSettings.getStreamTemplate("EVASIONE.xml");
            if (factory == null) {
                return;
            }
            righeDistintaCaricate = new HashMap<String, RigaDistintaCarico>();
            ...
            while ((rigaEvasione = (RigaEvasione) in.read()) != null) {
                ...
                if (rigaEvasione.getTipoRecord() == 4) {
                    rdc.setQtaEvasa(ObjectUtils.defaultIfNull(rdc.getQtaEvasa(), 0.0) + rigaEvasione.getQta());
                    if ("CF".equals(rdc.getRigaArticolo().getArticolo().getUnitaMisura().getCodice())) {
                        rdc.setQtaEvasa(NumberUtils.createDouble(rigaEvasione.getPezziEvasi()));
                    }
                    ...
                    rdc.setForzata(rigaEvasione.getQta() < 1.0);
                    boolean isRigaPezzi = isRigaPezzi(rigaEvasione.getNumRiga(),
                            rdc.getRigaArticolo().getAreaOrdine().getRiferimentiOrdine().getNumeroOrdine());
                    for (AttributoRiga attributoRiga : rdc.getRigaArticolo().getAttributi()) {
                        // Se è un ordine COOP inserisco sempre i pezzi, poi continuo con il ciclo
                        // normale
                        if (rdc.getRigaArticolo().getAreaOrdine().getDocumento().getEntita().getCodice().toString()
                                .equals(clienteCoop)
                                && CosaroSettingsBean.PEZZI_ATTRIBUTO
                                        .equals(attributoRiga.getTipoAttributo().getCodice())) {
                            attributoRiga.setValore(rigaEvasione.getPezziEvasi());
                        }
                        if (isRigaPezzi && CosaroSettingsBean.PEZZI_ATTRIBUTO
                                .equals(attributoRiga.getTipoAttributo().getCodice())) {
                            attributoRiga.setValore(rigaEvasione.getPezziEvasi());
                        } else if (!isRigaPezzi && CosaroSettingsBean.COLLI_ATTRIBUTO
                                .equals(attributoRiga.getTipoAttributo().getCodice())) {
                    ...
                    // sostituisco le note ( che in cosaro sono la linea di produzione usata per la
                    // prezzatura da
                    // inviare alle bilance ) con la descrizione estesa degli articoli ( giro
                    // normale della crea riga
                    // articolo )
                    Articolo articolo;
                    try {
                        articolo = panjeaDAO.load(Articolo.class, rdc.getRigaArticolo().getArticolo().getId());

                        DescrizioneLinguaArticoloEstesa descrizioneEstesa = articolo.getDescrizioniLinguaEstesa()
                                .get("it");
                        if (descrizioneEstesa != null) {
                            rdc.getRigaArticolo().setNoteRiga(descrizioneEstesa.getDescrizione());
                        }
                    } catch (Exception e) {
                        logger.error("--> Errore nel settare le note dell'articolo.", e);
                    ...
                    if ("CF".equals(rdc.getRigaArticolo().getArticolo().getUnitaMisura().getCodice())) {
                        rigaDistintaCaricoLotto.setQuantita(NumberUtils.createDouble(rigaEvasione.getPezziEvasi()));
                    }
            ...
            // creo il carico automatico solo se non ho attiva l'importazione dei documenti di
            // produzione da gammameat
     ...
     * @param numeroOrdine
     *            .
     * @return true se la riga è stata spedita per l'evasione di pezzi. False era un'evasione per
     *         colli
     */
    public boolean isRigaPezzi(int numRiga, String numeroOrdine) {
        // Potrei avere due ordini con lo stesso numero
        // prendo il più recente
        Query query = panjeaDAO.prepareQuery(
                "select r from RigaFileCoop r where r.numeroOrdine=:numeroOrdine and r.numeroRigaPezzi=:numeroRiga order by r.dataOrdine desc");
        query.setMaxResults(1);
        query.setParameter("numeroOrdine", numeroOrdine);
        query.setParameter("numeroRiga", numRiga);
        try {
            RigaFileCoop rigaFileCoop = (RigaFileCoop) panjeaDAO.getSingleResult(query);
            return rigaFileCoop.getNumeroRigaPezzi() == numRiga;
        } catch (DAOException e) {
            return false;
        }
    }
