...
import it.eurotn.panjea.onroad.domain.CondizRigaContratto;
import it.eurotn.panjea.onroad.domain.CondizRigaListino;
...
@Stateful(mappedName = "Panjea.OnRoadCONDIZExporter")
@SecurityDomain("PanjeaLoginModule")
@PermitAll
@LocalBinding(jndiBinding = "Panjea.OnRoadCONDIZExporter")
public class OnRoadCONDIZExporter extends AbstractDataExporter implements DataExporter {
	...
	public static final String BEAN_NAME = "Panjea.OnRoadCONDIZExporter";
	...
	private Map<Integer, String> caricaCategorieCommercialiArticolo() {
		...
		categorieCommercialiArticolo = null;
	...
	private Map<Integer, String> caricaCodiciSedeEntitaSedeMagazzino() {
		...
		Query query = panjeaDAO.prepareQuery(sb.toString());
		List<Object[]> codiciSede = new ArrayList<Object[]>();
		try {
			codiciSede = panjeaDAO.getResultList(query);
		} catch (DAOException e) {
			throw new RuntimeException(e);
		}
		Map<Integer, String> collectionToMap = collectionToMap(codiciSede,
				new IntegerStringKeyValueFromObjectProvider());
		codiciSede = null;
		return collectionToMap;
	}
	...
	private void esportaContratti(BeanWriter out) {
		...
		for (RigaContratto rigaContratto : righeContratto) {
			...
			if (rigaContratto.getId().intValue() == 1030) {
		...
		for (Listino listino : listini) {
			if (codiciListinoDaEsportare.isEmpty() || codiciListinoDaEsportare.indexOf(listino.getCodice()) != -1) {
				...
				for (RigaListino rigaListino : versione.getRigheListino()) {
					CondizRigaListino condizRigaListino = new CondizRigaListino(rigaListino);
					out.write(condizRigaListino);
					condizRigaListino = null;
				}
			}
		}
	}
	...
	private List<String> getCodiciListinoDaEsportare() {
		...
		try {
			...
			query.setParameter("paramChiave", "onRoadExportListiniList");
