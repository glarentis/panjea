...
/**
 * The CompoundIcon will paint two, or more, Icons as a single Icon. The Icons are painted in the order in which they
 * are added.
 *
...
public class CompoundIcon implements Icon {
    ...
    /**
     * Convenience contructor for creating a CompoundIcon where the gap is 0 and the X/Y alignments will default to
     * CENTER.
     *
     * @param axis
     *            the axis used to lay out the icons for painting. Must be one of the Axis enums: X_AXIS, Y_AXIS,
     *            Z_Axis.
     * @param icons
     ...
     *
     * @param axis
     *            the axis used to lay out the icons for painting Must be one of the Axis enums: X_AXIS, Y_AXIS, Z_Axis.
     * @param gap
     *            the gap between the icons
     * @param alignmentX
     *            the X alignment of the icons. Common values are LEFT, CENTER, RIGHT. Can be any value between 0.0 and
     *            1.0
     * @param alignmentY
     *            the Y alignment of the icons. Common values are TOP, CENTER, BOTTOM. Can be any value between 0.0 and
     *            1.0
     * @param icons
     *            the Icons to be painted as part of the CompoundIcon
     */
    ...
    /**
     * Convenience contructor for creating a CompoundIcon where the icons are layed out on on the X-AXIS, the gap is 0
    ...
    /**
     * When the icon value is smaller than the maximum value of all icons the icon needs to be aligned appropriately.
     * Calculate the offset to be used when painting the icon to achieve the proper alignment.
     *
     * @param maxValue
     *            .
     * @param iconValue
     ...
     */
    @Override
    public void paintIcon(Component component, Graphics ggraphics, int posx, int posy) {
        if (axis == Axis.X_AXIS) {
            int height = getIconHeight();

            for (Icon icon : icons) {
                int iconY = getOffset(height, icon.getIconHeight(), alignmentY);
                icon.paintIcon(component, ggraphics, posx, posy + iconY);
                posx += icon.getIconWidth() + gap;
            }
            ...
            for (Icon icon : icons) {
                ...
                icon.paintIcon(component, ggraphics, posx + iconX, posy);
                posy += icon.getIconHeight() + gap;
            }
        } else {
            int width = getIconWidth();
            int height = getIconHeight();
            ...
            for (Icon icon : icons) {
                int iconX = getOffset(width, icon.getIconWidth(), alignmentX);
                int iconY = getOffset(height, icon.getIconHeight(), alignmentY);
                icon.paintIcon(component, ggraphics, posx + iconX, posy + iconY);
            }
        }
    }
}
