...
public class FormattedTextFieldAdapter extends AbstractValueModelAdapter
    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        // Se ho 2 BigDecimal devo verificare con il compare se le proprietà sono cambiate
        if (e.getNewValue() instanceof BigDecimal) {
            BigDecimal newBigDecimalValue = e.getNewValue() == null ? BigDecimal.ZERO : (BigDecimal) e.getNewValue();
            BigDecimal oldBigDecimalValue = e.getOldValue() == null ? BigDecimal.ZERO : (BigDecimal) e.getOldValue();
            if (newBigDecimalValue.compareTo(oldBigDecimalValue) == 0) {
                return;
            }
        }
