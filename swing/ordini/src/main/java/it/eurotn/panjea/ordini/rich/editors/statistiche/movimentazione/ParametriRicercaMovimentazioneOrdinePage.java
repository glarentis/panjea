...
import it.eurotn.panjea.ordini.rich.forms.statistiche.movimentazione.ParametriRicercaMovimentazioneOrdiniForm;
...
public class ParametriRicercaMovimentazioneOrdinePage extends FormBackedDialogPageEditor implements InitializingBean {
	...
	private class LoadMovimentazioneCommand extends ActionCommand {
		...
		protected void doExecuteCommand() {
			...
			ParametriRicercaMovimentazioneOrdinePage.this.firePropertyChange(IPageLifecycleAdvisor.OBJECT_CHANGED,
	...
	private class ResetParametriRicercaCommand extends ActionCommand {
		...
		@Override
		protected void doExecuteCommand() {
			ParametriRicercaMovimentazioneOrdinePage.logger.debug("--> Reset command");
			// lancio null Object_changed per ripulire i risultati
			ParametriRicercaMovimentazioneOrdinePage.this.toolbarPageEditor.getNewCommand().execute();
			ParametriRicercaMovimentazioneOrdinePage.this.firePropertyChange(IPageLifecycleAdvisor.OBJECT_CHANGED,
					null, null);
			((ParametriRicercaMovimentazioneOrdiniForm) getBackingFormPage()).getFormModel().setReadOnly(false);
		}

	}

	public static final String PAGE_ID = "parametriRicercaMovimentazioneOrdinePage";
	private static Logger logger = Logger.getLogger(ParametriRicercaMovimentazioneOrdinePage.class);
	 ...
	 * Costruttore.
	 */
	public ParametriRicercaMovimentazioneOrdinePage() {
		super(PAGE_ID, new ParametriRicercaMovimentazioneOrdiniForm(new ParametriRicercaMovimentazione()));
		new PanjeaFormGuard(getBackingFormPage().getFormModel(), getLoadMovimentazioneCommand());
	}

	@Override
	public void afterPropertiesSet() throws Exception {
		org.springframework.util.Assert.notNull(ordiniDocumentoBD, "ordiniDocumentoBD cannot be null !");
		ParametriRicercaMovimentazioneOrdiniForm form = (ParametriRicercaMovimentazioneOrdiniForm) getBackingFormPage();
